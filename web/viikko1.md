...
Siirry hakemistoon. Komennolla <code>tree</code> saat esikuvan mitä kaikkea new-generaattorin suorittaminen sai aikaan. Huom: OSX:ssä ei ole oletusarvoisesti asennettuna tree-komentoa. Saat asennettua treen [homebrew:llä](http://brew.sh/) komennolla <code>brew install tree</code>
...
Alussa saattaa olla hieman sekavaa milloin ja missä käytetään yksikkö- ja milloin monikkomuotoa, miten tiedostot on nimetty ja mikä niiden sijainti on. Pikkuhiljaa kuitenkin käytänteet juurtuvat selkärankaan  alkavat vaikuttamaan loogisilta.
...
Syynä virheelle on se, että panimot tallettavan tietokantataulun luomisesta huolehtiva tietokantamigratio on suorittamatta.
...
Komento <code>Brewery.all</code> siis näyttää kaikki tietokannassa olevat panimot. Konsoli näyttää ensin tietokantaoperaation aiheuttaman SQL-kyselyn ja sen jälkeen kannasta saatavan panimo-oliot. Komento <code>Brewery.count</code> näyttää kannassa olevien paimoiden määrän.
...
Listaa nyt panimot ja lukumäärä tarkista niiden lukumäärä metodeilla <code>Brewery.all</code> ja <code>Brewery.count</code>. Huomaat, että vaikka loimme uuden olion, ei se mene kuitenkaan tietokantaan!
...
Tutustumme taulukon generointiin kohta hieman tarkemmin. Lisätään ensin sivulle (eli erb-templateen) ensin tieto panimoiden yhteenlasketusta määrästä. Eli lisää johonkin kohtaan sivua, esim. heti h1-tagien sisällä olevan otsikon jälkeen seuraava rivi
...
Palataan sitten tarkemmin HTML-taulukon muodostavaan koodiin. Jokainen panimo tulostuu omalle riviilleen taulukkoon Rubyn <code>each</code>-iteraattoria käyttäen:
...
Kaikkien panimoiden URLin (tai oikeastaan vaan URLin jälkiosan) generoi metodi <code>breweries_path</code>, yksittäisen panimon URL saadaan generoitua metodilla <code>brewery_path(id)</code>, missä parametrina on linkin kohteena olevan panimon id.
